...
with g.as_default():
	...
	with tf.variable_scope('FEATURE_EXTRACTOR_L'):
		...
		conv_l4 = conv_layer(conv_l3, [1, 1, 64, 40], [1, 1, 1, 1], name='CONV_L4')
		flatten = flatten_layer(conv_l4, 'FLATTEN_L')
		fc_l1 = fc_layer(flatten, [1440, 512], name='FC_L1')
		keep_prob = tf.placeholder(tf.float32)
		drop_l = tf.nn.dropout(fc_l1, keep_prob)
		fc_l2 = fc_layer(drop_l, [512, 128], name='FC_L2')
		l2_norm_l = tf.nn.l2_normalize(fc_l2, axis=1)
		
	softmax = fc_layer(fc_l2, [128, 10], False, 'FC_SOFTMAX_L1')
	...
	learning_rate = tf.where(tf.greater_equal(global_step, 10000),
							tf.train.polynomial_decay(1e-4, global_step,
														18000, 4e-6,
														power=1.0),
							1e-4)
	...
	second_train_ops = tf.train.AdamOptimizer(learning_rate).minimize(cross_entropy_loss, global_step=global_step, var_list=second_train_vars)
	...
	for step in range (30000):
		batch_x, batch_y = a.next_batch(50)
		_x = []
		_y_onehot = []
		for j in range (50):
			onehot = np.zeros(10)
